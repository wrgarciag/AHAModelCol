...
# countrylist <- read.csv(paste0(wd_dinpu,"super_regions.csv"), stringsAsFactors=FALSE)%>%filter(location!="Global", 
...
# 
# 
# names<-read.csv(paste0(wd_dinpu,"Country_groupings_extended.csv"), stringsAsFactors = F)%>%
#   select(location_gbd, gbd2019, iso3)%>%
#   rename(location = location_gbd)

#add covid mx data
load(paste0(wd_dinpu,"wpp.adj.Rda"))
...
#b_rates <- saveRDS(b_rates,file=paste0(wd_dinpu,"base_rates_2022_Col.rds"))

b_rates <- readRDS(file=paste0(wd_dinpu,"base_rates_2022_Col.rds"))
b_rates<-left_join(b_rates, wpp.adj%>%
                     rename(location = location_name)%>%
                     select( -Nx, -mx, -iso3))

#add location-specific CFAARC estimates
df<-read.csv(paste0(wd_dinpu,"IHME-CFR.csv"), stringsAsFactors = F)
...
pop20<-read.csv(paste0(wd_dinpu,"PopulationsAge20_2050.csv"), stringsAsFactors = F)
...
data.in<-fread(paste0(wd_dinpu,"bp_data6.csv"))%>%rename(location = location_gbd)%>%select(-Year, -Country)
...
#add scale-up data
inc<-read.csv("C:/Users/wrgar/OneDrive - UW/02Work/AHAPiCol/AHAModelCol/Model/covfxn2.csv", stringsAsFactors = F)%>%
...
data.in <- data.in[location=="Colombia",]
